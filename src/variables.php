...
/**
 * The qtype_numericalrecit_variables class is used to parse and evaluate variables.
 *
...
class qtype_numericalrecit_utils{
 ...
 */
class qtype_numericalrecit_variables {
    ...
    public function vstack_get_serialization(&$vstack) {
        ...
        foreach ($vstack->all as $name => $data) {
            if (array_key_exists($data->type, $ctype)) {
                ...
                if ($data->type == 's' || $data->type == 'ls')  for ($i = 0; $i < qtype_numericalrecit_utils::mycount($values); $i++)
    ...
    public function vstack_update_variable(&$vstack, $name, $index, $type, $value) {
        if ($index === null) {
            if ($type[0] == 'l') {  // Error check for list.
                ...
                if (qtype_numericalrecit_utils::mycount($value) < 1 || qtype_numericalrecit_utils::mycount($value) > self::$listmaxsize)  throw new Exception(get_string('error_vars_array_size', 'qtype_numericalrecit'));
                if (!is_numeric($value[0]) && !is_string($value[0]))  throw new Exception(get_string('error_vars_array_type', 'qtype_numericalrecit'));
                if ($type[1] == 'n') {
                    for ($i = 0; $i < qtype_numericalrecit_utils::mycount($value); $i++) {
            ...
            $index = intval($index);
            if ($index < 0 || $index >= qtype_numericalrecit_utils::mycount($list->value))  throw new Exception(get_string('error_vars_array_index_out_of_range', 'qtype_numericalrecit'));
    ...
    private function vstack_mark_current_top(&$vstack) {
        return (object)array('idcounter' => $vstack->idcounter, 'sz' => qtype_numericalrecit_utils::mycount($vstack->all));
    }
    ...
    public function substitute_variables_in_text(&$vstack, $text) {
        ...
        foreach ($ts as $text) {
            ...
            for ($i = 1; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2) {
                try {
    ...
    private function substitute_placeholders_in_text(&$vstack, $text) {
        ...
        for ($i = 1; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2)      // The length will always be odd, and the placeholder is stored in odd index.
    ...
    private function substitute_vname_by_variables(&$vstack, $text) {
        ...
        for ($i = 1; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2) {    // The length will always be odd, and the numbers are stored in odd index.
            $data = $this->vstack_get_variable($vstack, $splitted[$i]);
            if ($data->type == 'v') {
                $tmp = $this->vstack_get_variable($vstack, $data->value);
                if ($tmp === null)  throw new Exception(get_string('error_vars_undefined', 'qtype_numericalrecit', $data->value) . ' in substitute_vname_by_variables');
    ...
    private function substitute_strings_by_placholders(&$vstack, $text) {
        ...
        foreach ($splitted as $i => &$s)  if ($i % 2 == 1) {
            ...
            throw new Exception(get_string('error_forbid_char', 'qtype_numericalrecit'));
        return implode('', $splitted);
    }
    ...
    private function substitute_fixed_ranges_by_placeholders(&$vstack, $text) {
        ...
        for ($i = 1; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2) {    // The length will always be odd, and the numbers are stored in odd index.
            ...
            for ($z = $res->element[0]; $z < $res->element[1]; $z += $res->element[2]) {
                $data[] = $z;
                if (qtype_numericalrecit_utils::mycount($data) > self::$listmaxsize)  throw new Exception(get_string('error_vars_array_size', 'qtype_numericalrecit'));
            }
            if (qtype_numericalrecit_utils::mycount($data) < 1)  throw new Exception(get_string('error_vars_array_size', 'qtype_numericalrecit'));
    ...
    private function substitute_functions_by_placeholders(&$vstack, $text, $internal=false) {
        ...
        for ($i = 1; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2) {    // The length will always be odd, and the variables are stored in odd index.
    ...
    private function parse_fixed_range(&$vstack, $expression) {
        ...
        if (qtype_numericalrecit_utils::mycount($ex) > 3)  return null;
        $numpart = qtype_numericalrecit_utils::mycount($ex);
        for ($i = 0; $i < $numpart; $i++) {
            $ex[$i] = trim($ex[$i]);
            if (qtype_numericalrecit_utils::mycount($ex[$i]) == 0)  return null;
        ...
        if (qtype_numericalrecit_utils::mycount($ex) == 1)  $ex = array($ex[0], $ex[0] + 0.5, 1.);
        if (qtype_numericalrecit_utils::mycount($ex) == 2)  $ex = array($ex[0], $ex[1], 1.);
    ...
    public function parse_random_variables($text) {
        ...
        foreach ($assignments as $acounter => $assignment) {
            try {
                ...
                $name = trim($ex[0]);
                if (qtype_numericalrecit_utils::mycount($ex) == 1 && strlen($name) == 0) {
                    continue;   // If empty assignment.
                }
                if (qtype_numericalrecit_utils::mycount($ex) != 2) {
                    throw new Exception(get_string('error_syntax', 'qtype_numericalrecit'));
                }
                if (!preg_match('/^[A-Za-z0-9_]+$/', $name)) {
                    throw new Exception(get_string('error_vars_name', 'qtype_numericalrecit'));
                }
                ...
                if ($expression[0] == '{') {
                    ...
                    if ($bracket === null) {
                        throw new Exception(get_string('error_vars_bracket_mismatch', 'qtype_numericalrecit'));
                    }
                    ...
                    foreach ($bracket->expressions as $i => $ele) {
                        ...
                        if ($type != 'n') {
                            ...
                            if ($i > 0 && $result->type != $type)  throw new Exception(get_string('error_randvars_type', 'qtype_numericalrecit'));
                            $element = $result->value;
                            $numelement = 1;
                        }
                        if ($type == 'n') { // Special handle for number, because it can be specified as a range.
                            $result = $this->parse_fixed_range($vstack, $ele);
                            if ($result === null)  throw new Exception(get_string('error_syntax', 'qtype_numericalrecit'));
                            $element = $result->element;
                            $numelement = $result->numelement;
                        }
                        if ($i == 0) {
                            $listsize = $type[0] == 'l' ? qtype_numericalrecit_utils::mycount($element) : 1;
                        }
                        if ($i > 0) {
                            if (($type[0] == 'l' ? qtype_numericalrecit_utils::mycount($element) : 1) != $listsize) {
                    ...
                    $type = 'zh'.$result->type;
                    $var->numelement = qtype_numericalrecit_utils::mycount($result->value);   // The actual number should be a!, but it will not be used anyway.
                ...
                if ($var->numelement < 2) {
                    throw new Exception(get_string('error_randvars_set_size', 'qtype_numericalrecit'));
                }
    ...
    private function evaluate_assignments_substituted(&$vstack, $subtext, &$acounter) {
        ...
        while ($cursor < strlen($subtext)) {
            ...
            if ($first !== null && $first->var->type == 'v' && $first->var->value == 'for') {   // Handle the for loop.
                ...
                if (qtype_numericalrecit_utils::mycount($h) == 1)  throw new Exception(get_string('error_forloop', 'qtype_numericalrecit'));
                ...
                if ($list->type[0] != 'l')  throw new Exception(get_string('error_forloop_expression', 'qtype_numericalrecit'));
                ...
                if ($nameindex === null)  throw new Exception(get_string('error_vars_name', 'qtype_numericalrecit'));
    ...
    private function evaluate_general_expression_substituted_recursively(&$vstack, $expression) {
        ...
        if (strlen($expression) == 0)     // Check whether expression is empty.
            throw new Exception(get_string('error_subexpression_empty', 'qtype_numericalrecit'));
    ...
    public function parse_algebraic_variable(&$vstack, $expression) {
        ...
        if ($bracket->closeloc != strlen($expression) - 1)  throw new Exception(get_string('error_algebraic_var', 'qtype_numericalrecit'));
        $numelement = 0;
        $elements = array();
        foreach ($bracket->expressions as $e) {
            $res = $this->parse_fixed_range($vstack, $e);
            if ($res === null)  throw new Exception(get_string('error_algebraic_var', 'qtype_numericalrecit'));
    ...
    private function handle_square_bracket_syntax(&$vstack, &$expression) {
        ...
        if (qtype_numericalrecit_utils::mycount($res->expressions) < 1 || qtype_numericalrecit_utils::mycount($res->expressions) > self::$listmaxsize)
        ...
        if ($data !== null) {   // If the square bracket has a variable before it.
            ...
            if ($list[0]->type != 'n' || qtype_numericalrecit_utils::mycount($list) > 1)
                throw new Exception(get_string('error_vars_array_index_nonnumeric', 'qtype_numericalrecit'));
            if ($list[0]->value < 0 || $list[0]->value >= qtype_numericalrecit_utils::mycount($data->var->value))
        ...
        for ($i = 0; $i < qtype_numericalrecit_utils::mycount($list); $i++)  $list[$i] = $list[$i]->value;
    ...
    private function handle_special_functions(&$vstack, &$expression) {
        ...
        $sz = qtype_numericalrecit_utils::mycount($types);
        ...
        switch ($data->value) {
            case 'fill':
                ...
                if ($N < 1 || $N > self::$listmaxsize)  throw new Exception(get_string('error_vars_array_size', 'qtype_numericalrecit'));
            ...
            case 'len':
                ...
                $this->replace_middle($vstack, $expression, $l, $r, 'n', strval(qtype_numericalrecit_utils::mycount($values[0])));
                return true;
            case 'pick':
            ...
            case 'sort':
                ...
                if (qtype_numericalrecit_utils::mycount($values[0]) != qtype_numericalrecit_utils::mycount($values[1]))  break;
            ...
            case 'sublist':
                ...
                foreach ($values[1] as $idx) {
                    $idx = intval($idx);
                    if ($idx >= 0 && $idx < qtype_numericalrecit_utils::mycount($values[0]))  $sub[] = $values[0][$idx];
            ...
            case 'inv':
                ...
                foreach ($values[0] as $i => $idx) {
                    $idx = intval($idx);
                    if ($idx >= 0 && $idx < qtype_numericalrecit_utils::mycount($values[0]))  $sub[$idx] = $i;
                    else throw new Exception(get_string('error_vars_array_index_out_of_range', 'qtype_numericalrecit'));
                }
            ...
            case 'poly':
                ...
                $pow = qtype_numericalrecit_utils::mycount($vals);
                $pp = '';
                foreach ($vals as $v) {
                    $pow--;
                    if ($v == 0) {
        ...
        throw new Exception(get_string('error_func_param', 'qtype_numericalrecit', $data->value));
    }
    ...
    private function evaluate_numerical_expression($vstacks, $expression, $functype='F') {
        ...
        $all = array_fill(0, qtype_numericalrecit_utils::mycount($vstacks), array());
        for ($i = 1; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2) {
            ...
            if ($data === null || ($data->type != 'n' && $data->type != $functype)) {
                throw new Exception(get_string('error_eval_numerical', 'qtype_numericalrecit'));
            }
            ...
            if ($data->type == 'n') {   // If it is a number, store in $a for later evaluation.
                ...
                for ($j = 1; $j < qtype_numericalrecit_utils::mycount($vstacks); $j++) {  // If it need to evaluate the same expression with different values.
        ...
        $replaced = $splitted;
        for ($i = 1; $i < qtype_numericalrecit_utils::mycount($replaced); $i += 2)  if ($replaced[$i][0] == '$')  $replaced[$i] = 1;  // Substitute a dummy value for testing.
        ...
        foreach ($all as $a) {
            ...
            if (!isset($res))  throw new Exception(get_string('error_eval_numerical', 'qtype_numericalrecit'));
    ...
    public function get_expressions_in_bracket($text, $start, $open, $bset=array('(' => ')', '[' => ']', '{' => '}')) {
        ...
        for ($i = $start; $i < strlen($text); $i++) {
            ...
            if (qtype_numericalrecit_utils::mycount($ostack) > 0) {
        ...
        for ($i = $i + 1; $i < strlen($text); $i++) {
            ...
            if ($text[$i] == ',' && qtype_numericalrecit_utils::mycount($ostack) == 1) {
    ...
    public function insert_multiplication_for_juxtaposition($vstack, $text) {
        $splitted = explode('`', preg_replace('/(@[0-9]+)/', '`$1`', $text));
        for ($i = 3; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2) {    // The length will always be odd: placeholder in odd index, operators in even index.
    ...
    public function compute_numerical_formula_difference(&$A, &$B, $cfactor, $gradingtype) {
        $diffs = array();
        for ($i = 0; $i < qtype_numericalrecit_utils::mycount($B); $i++) {
    ...
    public function compute_algebraic_formula_difference(&$vars, $A, $B, $N=100) {
        if ($N < 1)  $N = 100;
        $diffs = array();
        for ($idx = 0; $idx < qtype_numericalrecit_utils::mycount($A); $idx++) {
            ...
            for ($i = 0; $i < qtype_numericalrecit_utils::mycount($nums); $i++)  $nums[$i] = $nums[$i] * $nums[$i];
    ...
    public function substitute_partial_formula(&$vars, $numericalrecit) {
        $res = array();
        for ($idx = 0; $idx < qtype_numericalrecit_utils::mycount($numericalrecit); $idx++) {
            ...
            for ($i = 1; $i < qtype_numericalrecit_utils::mycount($splitted); $i += 2) {
                ...
                if ($data->type == 'v') {
                    $tmp = $this->vstack_get_variable($vstack, $data->value);
                    if ($tmp === null)  throw new Exception(get_string('error_vars_undefined', 'qtype_numericalrecit', $data->value) . ' in substitute_partial_formula');
    ...
    public function find_formula_errors($formula) {
        ...
        while ( preg_match("~(^|[$safeoperatorchar,(])([a-z0-9_]*)\\(($operatorornumber+(,$operatorornumber+((,$operatorornumber+)+)?)?)?\\)~",
            ...
            switch ($regs[2]) {
                ...
                case 'pi':
                    if (strlen($regs[3]) != 0) {
                        return get_string('functiontakesnoargs', 'qtype_numericalrecit', $regs[2]);
                    }
                    break;
                ...
                case 'abs': case 'acos': case 'acosh': case 'asin': case 'asinh':
                    ...
                    if (strlen($regs[4]) != 0 || strlen($regs[3]) == 0) {
                        return get_string('functiontakesonearg', 'qtype_numericalrecit', $regs[2]);
                    }
                    break;
                ...
                case 'log': case 'round':
                    if (strlen($regs[5]) != 0 || strlen($regs[3]) == 0) {
                        return get_string('functiontakesoneortwoargs', 'qtype_numericalrecit', $regs[2]);
                    }
                    break;
                ...
                case 'atan2': case 'fmod': case 'pow': case 'ncr': case 'npr': case 'lcm': case 'gcd': case 'sigfig':
                    if (strlen($regs[5]) != 0 || strlen($regs[4]) == 0) {
                        return get_string('functiontakestwoargs', 'qtype_numericalrecit', $regs[2]);
                    }
                    break;
                ...
                case 'min': case 'max':
                    if (strlen($regs[4]) == 0) {
                        return get_string('functiontakesatleasttwo', 'qtype_numericalrecit', $regs[2]);
                    }
                    break;

                default:
                    return get_string('unsupportedformulafunction', 'qtype_numericalrecit', $regs[2]);
            }
